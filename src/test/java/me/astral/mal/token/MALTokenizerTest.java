...
public class MALTokenizerTest {
    ...
    @Test
    public void tokenizeMultilineComments(){
        MALTokenizer tokenizer = new MALTokenizer("//FirstLine\n//SecondLine");
        assertEquals(0, tokenizer.tokenize().size());
    }

    @Test
    public void tokenizeComments(){
        MALTokenizer tokenizer = new MALTokenizer("//This is a comment");
        assertEquals(0, tokenizer.tokenize().size());
    }

    @Test
    void tokenizeEmptySpace(){
        MALTokenizer tokenizer = new MALTokenizer("           // Empty");
        assertEquals(0, tokenizer.tokenize().size());
    }

    @Test
    void tokenizeLabel(){
        MALTokenizer tokenizer = new MALTokenizer(".label\tiadd1\t\t0x60");
        List<MALToken> tokens = tokenizer.tokenize();
        assertEquals(3, tokens.size());
        assertEquals(".label", tokens.get(0).value());
        assertEquals("iadd1", tokens.get(1).value());
        assertEquals("0x60", tokens.get(2).value());
    }

    @Test
    void tokenizeAllLabels(){
        MALTokenizer tokenizer = new MALTokenizer(".label\tnop1\t\t0x00\n" +
                ".label\tbipush1\t\t0x10\n" +
                ".label\tldc_w1\t\t0x13\n" +
                ".label\tiload1\t\t0x15\n" +
    ...
    @Test
    public void tokenizeIJVMDefinition(){
        MALTokenizer tokenizer = new MALTokenizer("// note that this is nearly identical to the example\n" +
